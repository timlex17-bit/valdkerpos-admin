<template>
          ...
          >
            ...
            <div class="mobile-info-grid">
              <div class="info-item">
                ...
                <span>{{ formatDateDisplay(purchase.purchase_date) }}</span>
              </div>
              <div class="info-item">
  ...
  const filteredGrandTotal = computed(() => {
    const total = filteredPurchases.value.reduce((sum, purchase) => sum + Number(purchase.grand_total || 0), 0)
  ...
  function generatePurchaseInvoice() {
    const maxId = purchases.value.length > 0 ? Math.max(...purchases.value.map((purchase) => purchase.id)) : 0
  ...
  .toolbar-grid {
    display: grid;
    grid-template-columns: minmax(240px, 1.8fr) minmax(180px, 1fr) minmax(180px, 1fr) auto;
    gap: 12px;
    align-items: center;
  }
