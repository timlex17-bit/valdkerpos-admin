<template>
  <div class="page">
    <section class="hero-card">
      <div class="hero-left">
        <div class="hero-badge">{{ t('bankLedgersPage.heroBadge') }}</div>
        <h1 class="page-title">{{ t('bankLedgersPage.title') }}</h1>
        <p class="page-subtitle">
          {{ t('bankLedgersPage.subtitle') }}
        </p>
      </div>
    ...
    <section class="stats-grid">
      <article class="stat-card">
        <p class="stat-label">{{ t('bankLedgersPage.totalEntries') }}</p>
        <h3 class="stat-value">{{ totalEntries }}</h3>
        <p class="stat-note">{{ t('bankLedgersPage.totalEntriesNote') }}</p>
      </article>

      <article class="stat-card">
        <p class="stat-label">{{ t('bankLedgersPage.totalIn') }}</p>
        <h3 class="stat-value money-in">{{ formatCurrency(totalIn) }}</h3>
        <p class="stat-note">{{ t('bankLedgersPage.totalInNote') }}</p>
      </article>

      <article class="stat-card">
        <p class="stat-label">{{ t('bankLedgersPage.totalOut') }}</p>
        <h3 class="stat-value money-out">{{ formatCurrency(totalOut) }}</h3>
        <p class="stat-note">{{ t('bankLedgersPage.totalOutNote') }}</p>
      </article>

      <article class="stat-card">
        <p class="stat-label">{{ t('bankLedgersPage.netFlow') }}</p>
        <h3 class="stat-value">{{ formatCurrency(netFlow) }}</h3>
        <p class="stat-note">{{ t('bankLedgersPage.netFlowNote') }}</p>
      </article>
    </section>
      ...
      <div class="toolbar-left">
        <div class="search-wrap">
          <input
            ...
            type="text"
            class="search-input"
            :placeholder="t('bankLedgersPage.searchPlaceholder')"
          />
        </div>

        <select v-model="accountFilter" class="filter-select">
          <option value="">{{ t('bankLedgersPage.allAccounts') }}</option>
        ...
        <select v-model="transactionTypeFilter" class="filter-select">
          <option value="">{{ t('bankLedgersPage.allTransactionTypes') }}</option>
        ...
        <select v-model="directionFilter" class="filter-select">
          <option value="">{{ t('bankLedgersPage.allDirections') }}</option>
          <option value="IN">{{ t('bankLedgersPage.directionIn') }}</option>
          <option value="OUT">{{ t('bankLedgersPage.directionOut') }}</option>
        </select>
      </div>

      <div class="toolbar-right">
        <div class="result-chip">{{ t('common.resultsCount', { count: filteredLedgers.length }) }}</div>
      </div>
    </section>
    ...
    <section class="table-card">
      <div class="table-header">
        <div>
          <h2>{{ t('bankLedgersPage.ledgerTransactions') }}</h2>
          <p>{{ t('bankLedgersPage.ledgerTransactionsNote') }}</p>
        </div>
      </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>{{ t('bankLedgersPage.account') }}</th>
              <th>{{ t('bankLedgersPage.transactionType') }}</th>
              <th>{{ t('bankLedgersPage.direction') }}</th>
              <th>{{ t('common.amount') }}</th>
              <th>{{ t('bankLedgersPage.before') }}</th>
              <th>{{ t('bankLedgersPage.after') }}</th>
              <th>{{ t('bankLedgersPage.invoiceOrder') }}</th>
          ...
          <tbody v-if="loading">
            <tr>
              <td colspan="10" class="empty-row">{{ t('bankLedgersPage.loading') }}</td>
            </tr>
          </tbody>

          <tbody v-else-if="filteredLedgers.length === 0">
            <tr>
              <td colspan="10" class="empty-row">{{ t('bankLedgersPage.empty') }}</td>
            </tr>
          </tbody>
            ...
            <tr v-for="item in filteredLedgers" :key="item.id">
              ...
              <td>
                <span class="direction-badge" :class="item.direction === 'IN' ? 'in' : 'out'">
                  {{ item.direction === 'IN' ? t('bankLedgersPage.directionIn') : t('bankLedgersPage.directionOut') }}
                </span>
              </td>
              ...
              <td>
                <div class="main-cell">
                  ...
                  <div class="cell-subtitle">
                    {{ t('bankLedgersPage.orderId') }}: {{ item.reference_order ?? '-' }}
                  </div>
                </div>
              </td>
    ...
    <div v-if="showDetailModal" class="modal-overlay" @click.self="closeDetailModal">
      <div class="modal-card">
        <div class="modal-header">
          <div>
            <h3>{{ t('bankLedgersPage.detailTitle') }}</h3>
            <p>{{ t('bankLedgersPage.detailSubtitle') }}</p>
          </div>
        ...
        <div class="modal-body">
          <div v-if="detailLoading" class="detail-loading">
            {{ t('bankLedgersPage.loadingDetail') }}
          </div>
          ...
          <div v-else-if="selectedLedger" class="detail-grid">
            ...
            <div class="detail-item">
              <label>{{ t('common.shopCode') }}</label>
              <p>{{ selectedLedger.shop_code || '-' }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.bankAccount') }}</label>
              <p>{{ selectedLedger.bank_account_name || '-' }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.transactionType') }}</label>
              <p>{{ getTransactionTypeLabel(selectedLedger.transaction_type) }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.direction') }}</label>
              <p>{{ selectedLedger.direction === 'IN' ? t('bankLedgersPage.directionIn') : t('bankLedgersPage.directionOut') }}</p>
            </div>
            ...
            <div class="detail-item">
              <label>{{ t('bankLedgersPage.balanceBefore') }}</label>
              <p>{{ formatCurrency(selectedLedger.balance_before) }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.balanceAfter') }}</label>
              <p>{{ formatCurrency(selectedLedger.balance_after) }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.referenceInvoice') }}</label>
              <p>{{ selectedLedger.reference_order_invoice || '-' }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.referenceOrder') }}</label>
              <p>{{ selectedLedger.reference_order ?? '-' }}</p>
            </div>

            <div class="detail-item">
              <label>{{ t('bankLedgersPage.referencePayment') }}</label>
        ...
        <div class="modal-footer">
          <button class="secondary-btn" @click="closeDetailModal">{{ t('common.close') }}</button>
        </div>
      </div>
    </div>
  </div>
...
function getTransactionTypeLabel(type: TransactionType | string) {
  switch (type) {
    case 'SALE_IN':
      return t('bankLedgersPage.transactionTypes.saleIn')
    case 'DEPOSIT':
      return t('bankLedgersPage.transactionTypes.deposit')
    case 'WITHDRAW':
      return t('bankLedgersPage.transactionTypes.withdraw')
    case 'TRANSFER_IN':
      return t('bankLedgersPage.transactionTypes.transferIn')
    case 'TRANSFER_OUT':
      return t('bankLedgersPage.transactionTypes.transferOut')
    case 'REFUND_OUT':
      return t('bankLedgersPage.transactionTypes.refundOut')
    case 'ADJUSTMENT':
      return t('bankLedgersPage.transactionTypes.adjustment')
    default:
      return type || '-'
  }
}
