...
async function fetchSummary() {
  const response = await axios.get(apiUrl(BACKUP_API.summary), {
...
async function fetchSettings() {
  const response = await axios.get(apiUrl(BACKUP_API.settings), {
    headers: getAuthHeaders(),
  })
...
async function fetchBackupHistory() {
  ...
  try {
    ...
    const response = await axios.get(apiUrl(BACKUP_API.history), {
      headers: getAuthHeaders(),
      params,
    })

    backupHistory.value = Array.isArray(response.data)
      ? response.data.map(mapBackupHistoryItem)
      : []
  } catch (error) {
...
async function saveBackupSettings() {
  ...
  try {
    ...
    const response = await axios.patch(apiUrl(BACKUP_API.settings), payload, {
      headers: getAuthHeaders(),
    })
...
async function runBackupNow() {
  ...
  try {
    ...
    const response = await axios.post(apiUrl(BACKUP_API.run), payload, {
      headers: getAuthHeaders(),
    })
...
async function viewBackup(item: BackupHistoryItem) {
  try {
    const response = await axios.get(apiUrl(BACKUP_API.detail(item.id)), {
      headers: getAuthHeaders(),
    })
...
async function confirmRestore() {
  ...
  try {
    const response = await axios.post(
      apiUrl(BACKUP_API.restore(restoreForm.backupId)),
      {
...
async function downloadBackup(item: BackupHistoryItem) {
  ...
  try {
    const response = await axios.get(apiUrl(BACKUP_API.download(item.id)), {
      headers: getAuthHeaders(),
      responseType: 'blob',
    })
...
async function deleteBackup(id: number) {
  ...
  try {
    await axios.delete(apiUrl(BACKUP_API.detail(id)), {
      headers: getAuthHeaders(),
    })
